# Quick gap_unit fix timing verification at 100MHz
...
set xdc_fh [open "${report_dir}/clk_${freq}_v2.xdc" w]
puts $xdc_fh "create_clock -period ${period} \[get_ports clk\]"
close $xdc_fh
read_xdc "${report_dir}/clk_${freq}_v2.xdc"
...
puts "  ${freq} post-gap-fix done in [expr {$elapsed/60}]m[expr {$elapsed%60}]s"

report_timing_summary -file "${report_dir}/${freq}_v2_timing.rpt"
report_utilization -file "${report_dir}/${freq}_v2_utilization.rpt"
write_checkpoint -force "${report_dir}/${freq}_v2_impl.dcp"
