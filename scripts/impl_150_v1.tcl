# 150MHz implementation — baseline with current v5 RTL (DONT_TOUCH + input pipes)
...
set xdc_fh [open "${report_dir}/clk_150MHz_v1.xdc" w]
puts $xdc_fh "create_clock -period 6.667 \[get_ports clk\]"
close $xdc_fh
read_xdc "${report_dir}/clk_150MHz_v1.xdc"

set TIME_start [clock seconds]

puts "============================================================"
puts "  150MHz baseline (v5 RTL: DONT_TOUCH + input pipes)"
puts "============================================================"
...
puts "  150MHz_v1 done in [expr {$elapsed/60}]m[expr {$elapsed%60}]s"

report_timing_summary -name "timing_1" -file "${report_dir}/150MHz_v1_timing.rpt"
report_utilization -file "${report_dir}/150MHz_v1_utilization.rpt"
write_checkpoint -force "${report_dir}/150MHz_v1_impl.dcp"
