# 100MHz implementation — approach B+A: DONT_TOUCH + input pipeline registers
...
set xdc_fh [open "${report_dir}/clk_100MHz_v5.xdc" w]
puts $xdc_fh "create_clock -period 10.000 \[get_ports clk\]"
close $xdc_fh
read_xdc "${report_dir}/clk_100MHz_v5.xdc"

set TIME_start [clock seconds]

puts "============================================================"
puts "  synth_design (B+A: DONT_TOUCH + input pipe registers)"
puts "============================================================"
synth_design -top soc_top -part xcku040-ffva1156-2-i -verbose
puts "  synth_design done: [expr {[clock seconds] - $TIME_start}]s"
...
puts "  100MHz_v5 done in [expr {$elapsed/60}]m[expr {$elapsed%60}]s"

report_timing_summary -name "timing_1" -file "${report_dir}/100MHz_v5_timing.rpt"
report_utilization -file "${report_dir}/100MHz_v5_utilization.rpt"
write_checkpoint -force "${report_dir}/100MHz_v5_impl.dcp"
